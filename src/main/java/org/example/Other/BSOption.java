...
public class BSOption {
    ...
    // defined constructor
    BSOption(double s, double k, double t, double r, double q, char type) {
        this.S = s;
        this.K = k;
        this.T = t;
        this.r = r;
    ...
    public double computePriceDiagram(double spot, double t) {
        ...
        double price;
        if (type == 'c')
            price = spot * CND(dplus) * Math.exp(-q * (T - t)) - K * CND(dminus) * Math.exp(-r * (T - t)); // price of a call option
        else
            price = spot * -1 * CND(-dplus) * Math.exp(-q * (T - t)) + K * CND(-dminus) * Math.exp(-r * (T - t));             // price of a put option
        return price;
    }
    ...
    public double computePriceVolStrike(double v, double strike, double t) {
        ...
        double priceRes;
        if (type == 'c')
            priceRes = S * CND(dplus) * Math.exp(-q * (T - t)) - strike * CND(dminus) * Math.exp(-r * (T - t)); // price of a call option
        else
            priceRes = S * -1 * CND(-dplus) * Math.exp(-q * (T - t)) + strike * CND(-dminus) * Math.exp(-r * (T - t));             // price of a put option
        return priceRes;
    }
    ...
    public static void main(String[] args) {
        ...
        BSOption b1=new BSOption(100,19000,15,2,10,'c');
        System.out.println(b1.computePrice());
